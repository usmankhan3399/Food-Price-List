...
const Dryfruits = () => {
  ...
  useEffect(() => {
   ...
   const Additem = () => {
     localStorage.setItem("category", "Livestock");
     navigate("/Addlivestock");
   };
   const data=()=>{
     axios
     .get(
       `http://192.168.43.93/FypApi/api/Fyp/GetLivestockList?cityname=${localStorage.getItem(
         "city"
       )}`
     )
     .then((res) => {
